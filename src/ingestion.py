...
def _is_safe_csv(df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # Check 1: Reasonable row count (not empty, not suspiciously large)
    if len(df) == 0:
        return False, "CSV file is empty"
    if len(df) > 100000:
        return False, "CSV file too large (max 100,000 rows)"
    
    # Check 2: Reasonable column count
    if len(df.columns) > 50:
        return False, "Too many columns (max 50)"
    if len(df.columns) < 2:
        return False, "Too few columns (minimum 2 required)"
    
    # Check 3: No executable content or scripts in column names
    for col in df.columns:
        col_str = str(col).lower()
        # Only flag actual dangerous patterns, not legitimate financial terms
        dangerous_patterns = [
    ...
    # Check 4: Check for suspicious content in first few rows
...
def _infer_date_column(df: pd.DataFrame) -> str:
    ...
    # Try to find date-like column by parsing
    for col in cols:
...
def _infer_merchant_column(df: pd.DataFrame) -> str:
    ...
    # Fall back to first text column after date/amount
    for col in cols:
...
def _merge_debit_credit(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Check if both debit and credit exist
    ...
    if has_debit and has_credit:
        ...
        # Merge: credit is positive, debit is negative
...
def validate_csv_structure(df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # Check if we can find date and amount columns
